...
//     try {
//         console.log('Sending request to update cart item:', reqData);
//         const { data } = await api.put(`/api/cart_items/${reqData.cartItemId}`, reqData.data);
//         dispatch({ type: UPDATE_CART_ITEM_SUCCESS, payload: data });
//     } catch (error) {
...
export const removeCartItem = (cartItemId) => async (dispatch) => {
    try {
        const response = await fetch(`/api/cart_items/${cartItemId}`, {
            method: 'DELETE',
            headers: {
                'Authorization': `Bearer ${yourJwtToken}`, // Thay thế bằng token JWT thực tế
        ...
        dispatch({ type: 'REMOVE_CART_ITEM_FAILURE', error });
    }
};