...
export const updateCartItem = (reqData) => async (dispatch) => {
    ...
    try {
        // Gọi API để cập nhật giỏ hàng, nhớ truyền vào đúng định dạng
        const { data } = await api.put(`/api/cart_items/${reqData.cartItemId}`, reqData.data, {
            headers: {
                Authorization: `Bearer ${yourJwtToken}` // Thay đổi với token của bạn
            }
        });
        dispatch({ type: UPDATE_CART_ITEM_SUCCESS, payload: data });
    } catch (error) {
        // Kiểm tra nếu có phản hồi từ server, lấy thông tin chi tiết
